...
def init_real_model():
    try:
        ...
        model = MMDetection3DModel(config_path, checkpoint_path, device=device)
        log(f"MMDetection3D模型加载成功: {model.class_names}")
        return model
    except Exception as e:
        log(f"MMDetection3D模型加载失败: {e}", "WARN")
        log("将使用模拟预测结果", "WARN")
        return None

real_model = init_real_model()
...
def mock_predict(pc_data, img_data, stage="clean"):
    ...
    if stage == "clean":
        idx = 0  # Car
        confidence = 0.85 + 0.1 * (base % 100) / 100
    elif stage == "adversarial":
        idx = 0  # 类别不变，但置信度大幅下降
        confidence = 0.45 + 0.1 * (base % 100) / 100
    else:  # defended
        idx = 0  # 类别恢复
        confidence = 0.78 + 0.1 * (base % 100) / 100
...
def run_prediction(pc_data, img_data, stage="clean"):
    if real_model is not None:
        try:
            ...
            log(f"真实模型推理失败: {e}，使用模拟结果", "WARN")
            return mock_predict(pc_data, img_data, stage)
    else:
        return mock_predict(pc_data, img_data, stage)

model_init_time = time.time() - start_time - data_load_time
...
def fgsm_attack_pointcloud(pc_data, epsilon):
    """
    FGSM点云攻击
    基于梯度符号的快速攻击，对所有点的坐标添加epsilon大小的扰动
    """
    log(f"执行点云FGSM攻击, epsilon={epsilon}")
    pc_adv = pc_data.copy()
    
    # 模拟FGSM攻击：在梯度方向（这里用随机符号近似）添加扰动
    # 实际ART中会计算真实梯度
    np.random.seed(42)
...
def sor_denoise_pointcloud(pc_data, nb_neighbors=20, std_ratio=2.0):
    ...
    if len(pc_data) < nb_neighbors:
        log(f"点云数量不足({len(pc_data)} < {nb_neighbors})，跳过SOR", "WARN")
        return pc_data
