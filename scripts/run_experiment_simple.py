...
def gaussian_noise_image(img: np.ndarray, sigma: float = 0.02) -> np.ndarray:
...
def process_sample(
    ...
    adv_img = gaussian_noise_image(img.astype(np.uint8), sigma=0.02)
    ...
    def_img = gaussian_blur_image(adv_img, kernel_size=5)
...
def generate_report(results: list[ExperimentResult], output_dir: Path):
    ...
    report_lines.append("Image attack: Gaussian noise")
    report_lines.append("Point cloud defense: Voxel downsampling")
    report_lines.append("Image defense: Gaussian blur")
    report_lines.append("")
...
def run_attack_defense_experiment(
    ...
    print("=" * 80)
    print("MULTIMODAL 3D OBJECT DETECTION ADVERSARIAL ROBUSTNESS EXPERIMENT")
    print("=" * 80)
    ...
    for i, sample_id in enumerate(sample_ids, 1):
        ...
        result = process_sample(str(pc_path), str(img_path), gt_class)
        results.append(result)
        print(f"  Ground truth: {result.gt_class}, Points: {result.point_count_clean}")
        print(f"  Adversarial points: {result.point_count_adversarial}")
        print(f"  Defended points: {result.point_count_defended}")
...
def main():
    parser = argparse.ArgumentParser(description="Windows Multimodal Adversarial Experiment")
    parser.add_argument("--kitti-dir", type=str, default="data/kitti", help="KITTI dataset directory")
    parser.add_argument("--output-dir", type=str, default="outputs/experiment_windows", help="Output directory")
